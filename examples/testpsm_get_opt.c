...
static uint64_t get_ctrl_tag(psm_net_ch_t *ch){
  return 0xdeadbeef;
}
...
int rwrite(psm_net_ch_t *ch, rdesc_t ret){
  ...
  if(ret.lbuf){
    psm_error_t code = psm_send_rdma_write(ch, NULL, 0, get_ctrl_tag(ch) );
    if(code != PSM_OK){
      fprintf(stderr, "rwrite() ctrl send failed\n");
      return -1;
    }
    code = psm_send_rdma_write(ch, ret.lbuf, ret.len, get_ch_tag(ch) );
    if(code != PSM_OK){
      fprintf(stderr, "rwrite() payload send failed\n");
      return -1;
    }
  } else {
...
void* rread(psm_net_ch_t *ch, uint32_t bytes){
  ...
  if(ret.lbuf){
    psm_mq_req_t req;
    psm_error_t code = psm_recv_rdma_read(ch, NULL, 0, get_ctrl_tag(ch), &req);
    if(code != PSM_OK){
      fprintf(stderr, "rread() ctrl tag recv failed\n");
      return NULL;
    }
    while(!comp){
      comp = progress_reqs(ch, &req);
    ...
    comp = 0;
    code = psm_recv_rdma_read(ch, ret.lbuf, bytes, get_ch_tag(ch), &req);
    if(code != PSM_OK){
      fprintf(stderr, "rread() data recv failed\n");
      return NULL;
    }
    while(!comp){
      comp = progress_reqs(ch, &req);
